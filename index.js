...
exports.handler = function(event, context, callback) {
    ...
    // overwrite config variables from cloudwatch event (if any)
    cloudwatchEventSetting(event);
...
function main(exchange, ticker) {
    ...
    Promise.all([getDBLastDonePromise, getExchangeLastDonePrice]).then(values => {
        ...
        if ((1 - lastDoneExchange / lastDoneDB) > config.percentageThreshold / 100) {
            ...
            // send email
            sendEmail(lastDoneDB, lastDoneExchange, exchange, ticker, percentageDropped);
        }
...
function sendEmail(lastDoneDB, lastDoneExchange, exchange, ticker, percentageDropped) {
    var ses = new AWS.SES({
        apiVersion: "2010-12-01"
    });

    var htmlData = "<b>" + ticker + " price fell by " + percentageDropped + "% on " + exchange + "</b>" +
    ...
    var eparam = {
        ...
        Message: {
            ...
            Subject: {
                Data: ticker + " price fell by " + percentageDropped + "% on " + exchange
            }
        },
...
function cloudwatchEventSetting(event) {
    if (typeof event.threshold !== 'undefined' && event.threshold) {
        config.percentageThreshold = event.threshold;
    }
    if (typeof event.binance !== 'undefined' && event.binance) {
        config.enableBinance = event.binance;
    }
    if (typeof event.binanceTicker !== 'undefined' && event.binanceTicker) {
        config.binanceTicker = event.binanceTicker;
    }
    if (typeof event.bittrex !== 'undefined' && event.bittrex) {
        config.enableBittrex = event.bittrex;
    }
    if (typeof event.bittrexTicker !== 'undefined' && event.bittrexTicker) {
        config.bittrexTicker = event.bittrexTicker;
    }
}